...
def get_search_results_working(query, fields: List[str], tabelas: List, count):
    ...
    text_fields = [field for field in fields if field not in ["codigo_tuss", "related_model.codigo_anvisa"]]

    long_fields = [field for field in fields if field in ["codigo_tuss", "related_model.codigo_anvisa"]]

    text_query = Q("multi_match", query=query, fields=text_fields, type='best_fields', fuzziness='AUTO')

    should_query = [text_query]

    long_query = Q('bool', should=[Q('match', **{field: int(query)}) for field in long_fields])
...
def get_search_results(query, fields: List[str], tabelas: List, count):
    ...
    text_query = Q("multi_match", query=query, fields=text_fields, type='best_fields', fuzziness=1)
    ...
    if tabelas:
        ...
        # Filter the documents based on the 'tabela' field
...
def get_suggestions(query, fields, tabelas):
    ...
    if fields == ['codigo_tuss'] or fields == ['related_model.codigo_anvisa'] or fields == ['codigo_tuss', 'related_model.codigo_anvisa']:
        return []
    ...
    s = s.query('bool', should=[Q('multi_match', query=word, type='phrase_prefix', fields=fields) for word in words])
